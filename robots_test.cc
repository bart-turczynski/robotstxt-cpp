...
bool IsUserAgentAllowed(const std::string_view robotstxt,
...
TEST(RobotsUnittest, GoogleOnly_LineTooLong) {
  ...
  // Disallow rule pattern matches the URL after being cut off at kMaxLineLen.
  {
    ...
    EXPECT_FALSE(IsUserAgentAllowed(
        robotstxt, "FooBot", std::string("http://foo.bar").append(longline).append("/fux")));
  }

  {
    ...
    EXPECT_TRUE(
        IsUserAgentAllowed(robotstxt, "FooBot",
                           std::string("http://foo.bar").append(longline_a).append("/qux")));
    // Matches cut off allow rule.
    EXPECT_TRUE(
        IsUserAgentAllowed(robotstxt, "FooBot",
                           std::string("http://foo.bar").append(longline_b).append("/fux")));
  }
}
